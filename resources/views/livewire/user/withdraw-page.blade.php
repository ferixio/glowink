        ...
        <section id="withdraw-form" class="bg-white rounded-lg shadow p-6">
            ...
            <form wire:submit.prevent="createWithdraw" class="space-y-4">
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    @if ($dataAdmin && $dataAdmin->bank_name && $dataAdmin->no_rek && $dataAdmin->bank_atas_nama)
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            <div>
                                <span class="text-gray-500">Bank:</span>
                                <span class="font-medium ml-2">{{ $dataAdmin->bank_name }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500">No. Rekening:</span>
                                <span class="font-medium ml-2">{{ $dataAdmin->no_rek }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500">Nama Rekening:</span>
                                <span class="font-medium ml-2">{{ $dataAdmin->bank_atas_nama }}</span>
                            </div>
                        </div>
                    @else
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <h3 class="text-sm font-medium text-yellow-800">
                                        Informasi Bank Admin Belum Lengkap
                                    </h3>
                                    <div class="mt-2 text-sm text-yellow-700">
                                        <p>Data rekening tujuan transfer belum tersedia. Silakan hubungi admin untuk
                                            melengkapi informasi bank.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <button type="submit" @if (!($dataAdmin && $dataAdmin->bank_name && $dataAdmin->no_rek && $dataAdmin->bank_atas_nama)) disabled @endif
                    style="width: 100%; background-color: {{ $dataAdmin && $dataAdmin->bank_name && $dataAdmin->no_rek && $dataAdmin->bank_atas_nama ? '#6B7280' : '#D1D5DB' }}; color: #ffffff; padding: 8px 16px; border: none; border-radius: 6px; font-weight: 600; cursor: {{ $dataAdmin && $dataAdmin->bank_name && $dataAdmin->no_rek && $dataAdmin->bank_atas_nama ? 'pointer' : 'not-allowed' }};"
                    @if (!($dataAdmin && $dataAdmin->bank_name && $dataAdmin->no_rek && $dataAdmin->bank_atas_nama)) title="Tidak dapat membuat withdraw karena data bank admin belum lengkap" @endif>
                    {{ $dataAdmin && $dataAdmin->bank_name && $dataAdmin->no_rek && $dataAdmin->bank_atas_nama ? 'Buat Permintaan Withdraw' : 'Data Bank Admin Belum Lengkap' }}
                </button>
            </form>
        </section>
    @else
